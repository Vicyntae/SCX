...
Function updateBodyPart(Actor akTarget, Bool abRapid = False)
  Int TargetData = SCXLib.getTargetData(akTarget)
  Float Total = calculateFatWeight(akTarget, TargetData)
...
Float Function calculateFatWeight(Actor akTarget, Int aiTargetData = 0)
  ...
  While i
    If i == "Level"
      FatRating += akTarget.GetLevel() * JMap.getFlt(JM_ActorValueWeights, i)
    Else
      ;FatRating += akTarget.GetActorValueMax(i) * JMap.getFlt(JM_ActorValueWeights, i)  ;Use this for SSE
      FatRating += akTarget.GetBaseActorValue(i) * JMap.getFlt(JM_ActorValueWeights, i)
    EndIf
    i = JMap.nextKey(JM_ActorValueWeights, i)
  EndWhile
