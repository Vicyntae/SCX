...
Function editBodyPart(Actor akTarget, Float afValue, String asMethodOverride = "", Int aiEquipSetOverride = 0, String asShortModKey = "SCX.esp", String asFullModKey = "Skyrim Character Extended")
  Note("Edit Weight Called! Target = " + akTarget.GetLeveledActorBase().GetName())
  If EnableMorph
...
Function updateBodyPart(Actor akTarget, Bool abRapid = False)
  Int TargetData = SCXLib.getTargetData(akTarget)
  Float Total = calculateWeightRating(akTarget, TargetData)
...
Float Function calculateWeightRating(Actor akTarget, Int aiTargetData = 0)
  ...
  Float WeightRating
  While i
    WeightRating += akTarget.GetBaseActorValue(i) * JMap.getFlt(JM_ActorValueRatings, i)
    i = JMap.nextKey(JM_ActorValueRatings, i)
  EndWhile
  If HealthRating != 0
    WeightRating += akTarget.GetBaseActorValue("Health") * HealthRating  ;Use this for LE
    ;WeightRating += akTarget.GetActorValueMax("Health") * HealthRating  ;Use this for SSE
  EndIf

  If StaminaRating != 0
    WeightRating += akTarget.GetBaseActorValue("Health") * StaminaRating  ;Use this for LE
    ;WeightRating += akTarget.GetActorValueMax("Health") * StaminaRating  ;Use this for SSE
  EndIf

  If MagickaRating != 0
    WeightRating += akTarget.GetBaseActorValue("Health") * MagickaRating  ;Use this for LE
    ;WeightRating += akTarget.GetActorValueMax("Health") * MagickaRating  ;Use this for SSE
  EndIf
  If LevelRating != 0
    WeightRating += akTarget.GetLevel() * LevelRating
  EndIf
  Return WeightRating
EndFunction
