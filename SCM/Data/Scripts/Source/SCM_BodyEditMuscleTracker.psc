ScriptName SCM_BodyEditMuscleTracker Extends ActiveMagicEffect

Actor MyActor
Int ActorData
Bool Gender ;False for male, True for female
SCM_BodyEditMuscle Property Muscle Auto
...
String Property DebugName
  String Function Get()
    If MyActor
      Return "[SCM_MuscleTracker " + MyActor.GetLeveledActorBase().GetName() + "] "
    Else
      Return "[SCM_MuscleTracker] "
    EndIf
  EndFunction
EndProperty
...
Event OnEffectStart(Actor akTarget, Actor akCaster)
  ...
  Else
    GoToState("Male")
  EndIf
  RegisterForModEvent("SCM_BodyEditMuscleSpellUpdate", "OnEditUpdate")
  RegisterForSingleUpdate(0.1)
EndEvent

Event OnEditUpdate(Form akTarget, Bool abRapid)
  ;Note("Edit update called! Target = " + (akTarget as Actor).GetLeveledActorBase().GetName())
  If akTarget == MyActor
    Gender = MyActor.GetLeveledActorBase().GetSex() as Bool
...
;/State Female
  ...
  Event OnUpdate()
    ...
    Int JM_Muscle = JMap.getInt(ActorData, "SCM_CBBEMuscleSliders")
    If !JValue.isExists(JM_Muscle) || !JValue.isMap(JM_Muscle)
      JM_Muscle = SCMSet.JM_SCM_CBBEMuscleSliders
    EndIf
    Float Inc = Muscle.Increment
    If RapidChange
      Inc *= 5
    EndIf
    ...
    ElseIf TargetValue < CurrentSize
      CurrentSize -= Inc
    EndIf
    String i = JMap.nextKey(JM_Muscle)
    While i
      NiOverride.SetBodyMorph(akTarget, i, ShortModKey, CurrentSize * (JMap.getFlt(JM_Muscle, i) / 100))
      i = JMap.nextKey(JM_Muscle, i)
    EndWhile
    NiOverride.UpdateModelWeight(akTarget)
    Float Rate = Muscle.IncrementRate
    If RapidChange
      Rate *= 5
    EndIf
...
State Male
  ...
  Event OnUpdate()
    Float TargetValue = JMap.getFlt(ActorData, "SCM_BodyEditMuscleTargetValue", 1)
    Float Samson = Muscle.SamsonWeighting
    Float Samuel = Muscle.SamuelWeighting
    Float Inc = Muscle.Increment
    If RapidChange
      Inc *= 10
    EndIf
    ...
    If RapidChange
      Rate *= 5
    EndIf
    RegisterForSingleUpdate(Muscle.Increment/Rate)
  EndEvent
EndState
Float SavedSamuel
Float SavedSamson
