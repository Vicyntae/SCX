ScriptName SCM_BodyEditFat Extends SCX_BaseBodyEdit

SCM_Library Property SCMLib Auto
SCM_Settings Property SCMSet Auto
Spell Property SCM_BodyEditFatSpell Auto

GlobalVariable Property SCX_BodyEditFatEnable Auto
Function reloadMaintenence()
  SCX_BodyEditFatEnable.SetValueInt(0)
  Utility.Wait(5)
  SCX_BodyEditFatEnable.SetValueInt(1)
EndFunction
...
Function editBodyPart(Actor akTarget, Float afValue, String asMethodOverride = "", Int aiEquipSetOverride = 0, String asShortModKey = "SCX.esp", String asFullModKey = "Skyrim Character Extended")
  If EnableMorph
    ...
    JMap.setFlt(TargetData, "SCM_BodyEditFatTargetValue", afValue)
    If !akTarget.HasSpell(SCM_BodyEditFatSpell)
      akTarget.AddSpell(SCM_BodyEditFatSpell, False)
    EndIf
    Int Handle = ModEvent.Create("SCM_BodyEditFatSpellUpdate")
...
Function updateBodyPart(Actor akTarget, Bool abRapid = False)
  Int TargetData = SCXLib.getTargetData(akTarget)
  Float Total = calculateFatRating(akTarget, TargetData)
...
Function addMCMOptions(SCX_ModConfigMenu MCM, Int JI_OptionIndexes)
  ...
  JIntMap.setStr(JI_OptionIndexes, MCM.AddSliderOption("Samson", SamsonWeighting, "{2}"), "BodyEdit." + asEdit + ".SamsonWeighting")
  JIntMap.setStr(JI_OptionIndexes, MCM.AddSliderOption("Samuel", SamuelWeighting, "{2}"), "BodyEdit." + asEdit + ".SamuelWeighting")
...
Function setHighlight(SCX_ModConfigMenu MCM, String asValue, Int aiOption)
  String Highlight
  If asValue == "Enable"
    HighLight = "Enables actors to gain or lose fat."
  ...
  ElseIf asValue == "SamsonWeighting"
    Highlight = "Sets how much Samson is affected"
  ElseIf asValue == "SamuelWeighting"
    Highlight = "Sets how much Samuel is affected"
  Else
    String[] ValueKeys = StringUtil.Split(asValue, "-")
    If ValueKeys[0] == "Stat"
      Highlight = "Set how much " + ValueKeys[1] + " affects fat"
    EndIf
  EndIf
...
{Set to recommended default values.}
  ...
  ElseIf asValue == "Increment"
    ...
    ValueArray[4] = 2
  ElseIf asValue == "SamsonWeighting"
    ValueArray[0] = SamsonWeighting
    ValueArray[1] = 1
    ValueArray[2] = 0.1
    ValueArray[3] = 0
    ValueArray[4] = 5
  ElseIf asValue == "SamuelWeighting"
    ValueArray[0] = SamuelWeighting
    ValueArray[1] = 1
    ValueArray[2] = 0.1
    ValueArray[3] = 0
    ValueArray[4] = 5
...
Function setSliderOptions(SCX_ModConfigMenu MCM, String asValue, Int aiOption, Float afValue)
  ...
  ElseIf asValue == "Increment"
    Increment == afValue
  ElseIf asValue == "SamsonWeighting"
    SamsonWeighting = afValue
  ElseIf asValue == "SamuelWeighting"
    SamuelWeighting = afValue
  Else
    String[] ValueKeys = StringUtil.Split(asValue, "-")
    If ValueKeys[1] == "Health"
...
Float Property SamsonWeighting Auto
Float Property SamuelWeighting Auto
...
Float Function calculateMuscleRating(Actor akTarget, Int aiTargetData = 0)
  ...
  Float MuscleRating
  While i
    MuscleRating += akTarget.GetBaseActorValue(i) * JMap.getFlt(JM_ActorValueRatings, i)
    i = JMap.nextKey(JM_ActorValueRatings, i)
  EndWhile
  If HealthRating != 0
    MuscleRating += akTarget.GetBaseActorValue("Health") * HealthRating  ;Use this for LE
    ;MuscleRating += akTarget.GetActorValueMax("Health") * HealthRating  ;Use this for SSE
  EndIf

  If StaminaRating != 0
    MuscleRating += akTarget.GetBaseActorValue("Health") * StaminaRating  ;Use this for LE
    ;MuscleRating += akTarget.GetActorValueMax("Health") * StaminaRating  ;Use this for SSE
  EndIf

  If MagickaRating != 0
    MuscleRating += akTarget.GetBaseActorValue("Health") * MagickaRating  ;Use this for LE
    ;MuscleRating += akTarget.GetActorValueMax("Health") * MagickaRating  ;Use this for SSE
  EndIf
  If LevelRating != 0
    MuscleRating += akTarget.GetLevel() * LevelRating
  EndIf
  Return MuscleRating
EndFunction

Float Function calculateFatRating(Actor akTarget, Int aiTargetData = 0)
  If !JValue.isMap(aiTargetData)
    aiTargetData = getTargetData(akTarget)
  EndIf
  Float FatRating
  String i = JMap.nextKey(JM_ActorValueRatings)
  While i
    FatRating += akTarget.GetBaseActorValue(i) * JMap.getFlt(JM_ActorValueRatings, i)
    i = JMap.nextKey(JM_ActorValueRatings, i)
  EndWhile
  If HealthRating != 0
    FatRating += akTarget.GetBaseActorValue("Health") * HealthRating  ;Use this for LE
    ;FatRating += akTarget.GetActorValueMax("Health") * HealthRating  ;Use this for SSE
  EndIf

  If StaminaRating != 0
    FatRating += akTarget.GetBaseActorValue("Health") * StaminaRating  ;Use this for LE
    ;FatRating += akTarget.GetActorValueMax("Health") * StaminaRating  ;Use this for SSE
  EndIf

  If MagickaRating != 0
    FatRating += akTarget.GetBaseActorValue("Health") * MagickaRating  ;Use this for LE
    ;FatRating += akTarget.GetActorValueMax("Health") * MagickaRating  ;Use this for SSE
  EndIf

  If SCMSet.SCN_Installed && SCMSet.UseSCNCalories
    FatRating += JMap.getFlt(aiTargetData, "SCNStoredCalories")
  EndIf

  If LevelRating != 0
    FatRating += akTarget.GetLevel() * LevelRating
  EndIf
  Return FatRating
EndFunction
