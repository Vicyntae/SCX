ScriptName SCVPerkDragonDevourer Extends SCX_BasePerk
...
Bool Function canTake(Actor akTarget, Int aiPerkLevel, Bool abOverride, Int aiTargetData = 0)
  ...
  If SCVLib.isPred(akTarget)
    Int DragonsKilled = DragonsAbsorbed.GetValueInt()
    Int CurrentDragonSouls = JMap.getInt(SCVLib.getTargetData(PlayerRef), "SCV_DragonGemsConsumed")
    Int Level = akTarget.GetLevel()
    Int NumEatenPrey = JMap.getInt(aiTargetData, "SCV_NumDragonsEaten")
    If aiPerkLevel == 1 && DragonsKilled >= 30 && Level >= 30 && MQ203.IsCompleted() ;Complete Quest Alduin's Wall
      Return True
    ElseIf aiPerkLevel == 2 && DragonsKilled >= 70 && Level >= 50 && NumEatenPrey >= 20 && MQ305.IsCompleted() == 200 ;Complete Quest Dragonslayer
      Return True
